...
def test_margin_dashboard():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   - Preview function: {'✅' if has_preview else '❌'}")
            print(f"   - Export function: {'✅' if has_export else '❌'}")
...
def main():
    ...
    if failed > 0:
        print("\n⚠️  Some tests failed. Checking deployment status...")
        # 배포 상태 확인
        try:
